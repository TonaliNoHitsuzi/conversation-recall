...
> **多域分布式知识检索 · 分级渐进 · 只读 · 自主优化**
> Tiered progressive retrieval over a distributed knowledge base — read-only, with self-optimizing data governance.

让 AI 在当前会话里检索过往**对话 / 周报 / 多个项目成果**，并把"知识到底有没有用"交给**真实任务结果**裁定，而非 AI 当下自评。

---

## 为什么需要它 / Why

AI 会话是易失的——每次新会话从零开始，过往讨论、踩过的坑、得出的结论都消失。本工具把历史知识建成**可检索的分布式知识库**，让 AI 能"回忆"：

| 域 | 内容 | 入索引（高信号）| 存库不索引（按需取）|
|----|------|----------------|---------------------|
| **对话** | opencode 历史会话 | text 正文 | reasoning / tool / patch / 子代理 |
| **周报** | 周报、摘要、产出文档、计划、修改记录 | section 正文（v3 按 genre 自动分类） | 素材（原始草稿）|
| **项目** × N | 各项目文件夹的 md（v3 支持任意数量，web 终端可在线新建） | section（策展入库，自动 genre 推断） | 附件指针 |
...
```
T1  search      正文/section 片段（多词并行，跨多域）   ← 唯一起点
 │
...
python scripts/recall.py search "词1" "词2" ... "词10" \
    [--domain 对话|周报|<项目slug>|all] \
    [--since YYYY-MM-DD] [--until YYYY-MM-DD] \
    [--not "排除词"] [--limit N]
```

- **多词并行**：每词每域独立 FTS5，跨域结果分域展示并标来源（`[对话]`/`[周报]`/`[<项目名>]`），共识度越高越靠前。
...
- **三栏工作台**（v3）：左侧库导航（含分组/收藏夹）→ 中间搜索结果 → 右侧详情阅读器，各自独立滚动
- **搜索栏 + 时间筛选 + 排除框 + genre 过滤**：覆盖 CLI 全部检索能力 + 按文档类型（周报正文/摘要/素材/技术文档/计划）精确过滤
- **结果展开 / 分级下钻**：点开看片段，再点拉全文
- **反馈打分 + 挂 issue + 收藏**：浏览器里直接裁决 + 跨库收藏夹
- **灰库视图**：看哪些被淘汰了，一键 revive
- **文件服务**（v3）：图片渲染 / PDF 下载 / 原 md 下载 / 代码块复制 / Mermaid PNG·SVG 导出 / LaTeX 公式复制
- **库管理**（v3）：web 界面直接新建项目库（原生文件夹选择器）、Edge 式分组管理
- **暗色模式**（v3）：三态循环（跟随系统/亮/暗），Mermaid + MathJax 同步换肤
- **关联文件跳转**（v3）：点开任意文档自动列出同目录/同周的伴随文件 + 引用图片缩略图
- **渲染**：Markdown / 代码高亮 / Mermaid 图 / LaTeX 公式（MathJax 字体走 jsdelivr CDN，需联网）
...
```
 ...
 项目域 projects/<slug>.db(外置盘) ×N ┘   ↑ 外置盘离线自动跳过，对话域常驻
                                       ↑ v3: 项目数量不限，web 终端可在线新建

 meta.db  ← rank / 反馈 / 勘误 / 审计日志 / 收藏 / 库分组
 gray.db  ← 失效归档，active 全空才兜底检索（仅项目域）
```
...
- ✅ **R1 地基**：多域分布式存储 + 统一查询 + document 闸门
- ✅ **R2 入库**：策展式入库 + 周报联动 + 文档域分层
- ✅ **R3 排名+反馈**：rank 融合 + 现实支点反馈 + 勘误浮现
- ✅ **R4 治理**：审计 + 灰库 + size 警报 + 淘汰/复活
- ✅ **检索增强**：时间过滤 / NOT 排除 / 浏览模式 / browse dedup 下沉 SQL
- ✅ **网页终端 v2**：本地人工入口 + 反馈打分 + 灰库视图 + 暗色模式 + 双栏独立滚
- ✅ **网页终端 v3**：三栏工作台 + genre 分类 + 收藏 + 库管理（在线新建/分组）+ 文件服务（图片渲染/PDF下载/Mermaid导出/LaTeX复制）+ 关联文件跳转 + 浏览器 bug 修复（dedup/路径/字体）
...
| 🔮 T3 | **跨域联动可视化**（对话→周报→项目的引用链）| 用户有跨域追溯需求（v3 已有 siblings 关联文件做基础）|
| 🔮 T4 | **多用户/多机同步**（db 合并协议）| 团队协作场景出现 |
| 🔮 T5 | **导入外部知识源**（RSS / 书签 / PDF 批量）| 配套 doc2md 已就绪，等需求 |
| ✅ T6 | ~~标签定向检索~~ | v3 已通过 genre 自动推断 + `?genre=` 过滤实现 |
| ✅ T7 | ~~非文本资产指针 + 渲染~~ | v3 已通过 /api/raw + 图片渲染 + PDF 下载实现 |
